...
divCreate.innerHTML = "Hello World";
...
btnCreate.innerHTML = "click";
document.body.appendChild(btnCreate);


// 3 2번에서 만든 버튼을 누르면 바탕화면 색이 바뀌게 하세요.

const change_BackgroundColor = function (e) {
  document.body.style.backgroundColor = "lightblue";
};
btnCreate.addEventListener("click",change_BackgroundColor);
...
for (let i=0; i<cars.length; i++) {
  ...
  liCreate.innerHTML = cars[i];
  olCreate.appendChild(liCreate);
};
document.body.appendChild(olCreate);
...
function Member(name, age, height) {
  this.name = name;   //부모가 가진 프로퍼티를 표현하기위해 this
  this.age = age;
  this.height = height;
  this.print = function () {
